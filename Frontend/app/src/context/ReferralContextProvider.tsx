...
const ReferralContextProvider: FC<any> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const setFees = async () => {
            ...
            let cachedReferrer: string | null | CachedReferrer = localStorage.getItem(REFERRER_CACHE_KEY);
            try {
            ...
            if (urlReferrer && !isAddress(urlReferrer)) {
                try {
                    const response = await fetch(`https://api.ensideas.com/ens/resolve/${urlReferrer}`);
            ...
            if (urlReferrer && urlReferrer !== (cachedReferrer as CachedReferrer)?.referrer) {
            ...
            setFeesOnTop(recipients.map((recipient) => `${recipient}:${REFERRAL_FEE_USD / recipients.length}`));
        };
        setFees();
    }, []);

    return <ReferralContext.Provider value={{ feesOnTop }}>{children}</ReferralContext.Provider>;
};
