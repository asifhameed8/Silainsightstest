...
const ChainContextProvider: FC<any> = ({ children }) => {
    ...
    useEffect(() => {
        const selectedChainId = localStorage.getItem('reservoir.chainId') || DefaultChain.id;
    ...
    // useEffect(() => {
    //     chain?.id && setGlobalChainId(chain?.id);
    //     chain?.id && localStorage.setItem('reservoir.chainId', `${chain?.id}`);
    // }, [chain?.id]);
    ...
    return <ChainContext.Provider value={{ chain: currentChain, switchCurrentChain }}>{children}</ChainContext.Provider>;
};
