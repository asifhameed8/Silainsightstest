...
class CloudinaryService {
    ...
    // optimize image
    optimizeImageURL({ src, width, quality = 'auto', transformation, fileType, objectFit }: IOptmizeProps) {
        ...
        if (fileType && fileType == 'gif') {
            return this.cld.image(src).delivery(format(gif()).lossy()).delivery(qualityFn(30)).setDeliveryType('fetch').setAssetType('image').toURL();
        }
        if (transformation) {
            return this.cld.image(src).setDeliveryType('fetch').namedTransformation(name(transformation)).toURL();
        }
        if (width && objectFit !== 'contain') {
            return this.cld.image(src).setDeliveryType('fetch').resize(fill().width(width)).delivery(qualityFn(quality)).toURL();
        } else {
    ...
    optimizeVideoURL({ src, width, resize = true, fileType }: IOptmizeProps) {
        ...
        // }
        if (fileType && fileType == 'gif') {
            return this.cld.video(src).delivery(format(videoMp4())).setDeliveryType('fetch').setAssetType('image').toURL();
