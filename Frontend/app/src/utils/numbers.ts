...
function formatNumber(amount: number | null | undefined | string, maximumFractionDigits: number = 2) {
 ...
 */
function formatBN(amount: BigNumberish | null | undefined, maximumFractionDigits: number, decimals: number = 18) {
    if (typeof amount === 'undefined' || amount === null) return '-';

    const amountToFormat = typeof amount === 'number' ? amount : +utils.formatUnits(amount, decimals);
    ...
    if (parts && parts.length > 0) {
        ...
        return typeof amount === 'string' || typeof amount === 'number' ? `${amount}` : '';
    }
}
...
export const bigNumberToDecimal = (value = 0, unitName?: string | BigNumberish) => {
    const etherValue = ethers.utils.formatUnits(value, unitName);
    return etherValue;
};

export const cryptoAmount = (value = 0, decimals = 18) => {
    return Number(value) / 10 ** decimals;
};

export const toFixedNumber = (value: number | string, fixedValue = 2) => {
    return value ? Number(value).toLocaleString(undefined, { maximumFractionDigits: fixedValue }) : '0';
};

export { formatDollar, formatBN, formatNumber, formatTime };
