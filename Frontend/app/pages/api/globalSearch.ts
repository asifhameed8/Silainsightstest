...
export type SearchCollection = NonNullable<paths['/search/collections/v1']['get']['responses']['200']['schema']['collections']>[0] & {
    chainName: string;
    chainId: number;
...
export default async function handler(req: Request) {
    ...
    supportedChains.forEach(async (chain) => {
        ...
        promises.push(fetcher(`${reservoirBaseUrl}/search/collections/v1`, query, headers));
    });
    ...
    if (isAddress) {
        const promises = supportedChains.map(async (chain) => {
            ...
            const { data }: any = await fetcher(`${reservoirBaseUrl}/collections/v5?contract=${query}&limit=6`, {}, headers);
            return data.collections.map((collection: Collection) => {
                const processedCollection: SearchCollection = {
                    ...
                    chainName: chain.routePrefix.toLowerCase(),
                    chainId: chain.id,
        ...
        if (results.length > 0) {
            ...
            let ensData = await fetch(`https://api.ensideas.com/ens/resolve/${query}`).then((res) => res.json());
            searchResults = [
                {
                    type: 'wallet',
                    data: {
    ...
    } else if (/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)?/gi.test(query as string)) {
        let ensData = await fetch(`https://api.ensideas.com/ens/resolve/${query}`).then((res) => res.json());
        ...
        const usdCoinPrices = await fetch(`${HOST_URL}/api/usdCoinConversion`).then((res) => res.json());

        const responses = await Promise.all(promises);
        responses.forEach((response, index) => {
            const chainSearchResults = response?.data.collections.map((collection: SearchCollection) => ({
                type: 'collection',
                data: {
                    ...collection,
                    ...
                    volumeCurrencyDecimals: supportedChains[index].nativeCurrency.decimals,
                    tokenCount: collection.tokenCount,
                    allTimeUsdVolume: (collection.allTimeVolume && collection.allTimeVolume * usdCoinPrices?.prices?.[index]?.current_price) || 0
                }
            }));
        ...
        if (usdCoinPrices) {
            searchResults = searchResults.sort((a, b) => b.data.allTimeUsdVolume - a.data.allTimeUsdVolume);
        }
    }
