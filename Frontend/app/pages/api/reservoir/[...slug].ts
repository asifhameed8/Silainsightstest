...
const proxy = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (endpoint.includes('redirect/')) {
        ...
        if ([mainnet.id as number, goerli.id].includes(chain.id) && endpoint.includes('currency')) {
            if (endpoint.includes(constants.AddressZero)) {
                res.redirect('/icons/currency/no-padding-eth.png');
                return;
            } else if (endpoint.includes(wrappedContracts['1'].toLowerCase()) || endpoint.includes(wrappedContracts['5'].toLowerCase())) {
